...
class StorageList {
    ...
    private FreeShell.FileAdapter fileAdapter;
    ...
    public StorageList(FreeShell freeShell, FreeShell.FileAdapter fileAdapter, FSync fsync) {
    ...
    synchronized List<File> getDisks() {
        try {
            ...
            File f = new File(freeShell.myDir());
            if (f.exists())
                files.add(f);
            return files;
        }catch (Throwable t){
    ...
    synchronized ArrayList<File> getStorageFileList(int abc) {
        try {
            ...
            sort(files, abc);
            File my = new File(freeShell.myHistoryDir());
            files.add(0, my);
            my = new File(freeShell.myHomeDir());
            files.add(0, my);
