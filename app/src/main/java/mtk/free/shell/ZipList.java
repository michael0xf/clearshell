...
public class ZipList {
    private FreeShell.ZipAdapter fileAdapter;
    ...
    public ZipList(FreeShell freeShell, FreeShell.ZipAdapter fileAdapter, FSync fsync) {
    ...
    synchronized void showZipList2(){
        freeShell.log("show storage list");
    ...
    synchronized void openFile(final File f, final boolean addParent, Integer position, Runnable run) {
        if (f == null){
            freeShell.log("Open null file");
        }else{
            freeShell.log("open file " + f.getAbsolutePath());
        }
    ...
    synchronized public File findFile(String s){
        if (dirs.size() > 0){
            ...
            if (file instanceof Zip){
                ...
                Toast.makeText(freeShell, "Error. ZipFile not found", Toast.LENGTH_LONG).show();
            }
        }
        return null;
    ...
    /*public void setBase(FreeShell.FileAdapter base) {
        dirs.clear();
    ...
    public class Zip extends File {
        ...
        public Zip(@NonNull String pathname, ZipFile zipFile) {
            super(pathname);
        ...
        public boolean delete(){
            freeShell.okDialog("It's archive.", "Deleting files from the archive is not supported.");
            return false;
        }
...
//        private InputStream inputStream;
        ...
        public File2(@NonNull String pathname, ZipFile zipFile, String path) throws IOException {
            super(pathname);
...
/*            byte[] buf = new byte[(int)ze.getSize()];
        ...
        public void extract(File dir){
            try {
                ...
                if (!f.exists()){
                    if (!f.createNewFile()){
                        Toast.makeText(freeShell, "Error. " + f.getAbsolutePath() + "Can't create file.", Toast.LENGTH_LONG).show();
                        return;
                    }
                }else{
                    Toast.makeText(freeShell, "Error. " + f.getAbsolutePath() + "File already exists.", Toast.LENGTH_LONG).show();
                    return;
                }
    ...
    public class Dir extends File implements Entry{
        ...
        public Dir(@NonNull String pathname, String full, File parent) {
            super(pathname);
            this.parent = parent;
        ...
        public void extract(File dir){
            ...
            if (! f.exists()){
                ...
                if (!f.isDirectory()){
                    freeShell.okDialog("Error", f.getAbsolutePath() + " not a directory.");
                    return;
                }
            }
            for(File file: files){
